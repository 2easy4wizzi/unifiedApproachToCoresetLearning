...
def get_argv():
    ...
    argv['global'] = {
        ...
        'bs_test': 256,
        'shuffle_test': False,
        'batch_interval': -1  # each x batches print batch stats
    }
    ...
    if argv['global']['mode'] == 'fs':
        c_model_base_path = './results/full_sweep_vgg19-cifar10_coreset'  # all 1 balance - no layers
    elif argv['global']['mode'] == 'lbl':
        c_model_base_path = './results/lbl_c{}_vgg19-cifar10_coreset'  # layer index will be inserted later by layer
    else:
        c_model_base_path = './bla.pt'

    argv['coreset'] = {
        'bs_train': 512,
        'shuffle': False,
        'epochs': 180,
        'opt_dict': ADAM_OPT_V1,
...
def main():
    ...
    if argv['global']['save_models'] and not os.path.exists(log_path):
        f = open(log_path, 'w')
        f.close()
    # TrainTestScript.test_loaders_check(train_dl_full, test_dl, p_model, argv['global']['f'])
