...
from DataReduction.LogisticReg import handlerLogisticRegressionV2 as Log
import math
...
def load_tmf_sp(tmf_path: str) -> np.array:
...
def test_Q_1_coreset_mean(f: Callable, Q: torch.Tensor, A: torch.Tensor, C: torch.Tensor, U: torch.tensor = None,
    ...
    if title is not None:
        ...
        print(msg.format(Q.shape[0], np.mean(lcs) / n, np.mean(las) / n, np.mean(ers), np.std(ers)))
...
def get_sp_list(A: torch.Tensor, argv: dict, group_names: list, trainQ: torch.Tensor, valQ: torch.Tensor,
    ...
    for group_name in group_names:
        ...
        if 'SGD' in group_name:  # SP stays none
            ...
            SP = load_tmf_sp(argv['TMF_sp_folder'])
...
def test_coresets(A: torch.Tensor, argv: dict, name_color_list, coresets_dict, testQ):
    ...
    # st_max_time = get_current_time()
    # title = title_base.format('strong test(|testQ|={})'.format(t_size_str(testQ)), STRONG_TITLE_MAX_MEAN)
    # save_path = '{}/{}_to_{}_strong'.format(folder, c_sizes[0], c_sizes[-1]) if save_plots else None
    # errors_dict = test_many_c_sizes_by_c(f, testQ, A, coresets_dict, use_max=True)
    # out_results(c_sizes, errors_dict, name_color_list, title, save_path, show_plots, with_std)
    # print('st_max_time {}'.format(get_time_str(st_max_time)))
    ...
    for epoch in range(1, argv['Bal']['epochs'] + 1):
        ...
        if argv['Bal']['epochs_info']:
            ...
            if argv['Bal']['epochs_info_stds']:
                msg += ' loss_c:{:,.05f}({:,.05f})'.format(np.mean(loss_c_ers) / n, np.std(loss_c_ers) / n)
                msg += ', loss_p:{:,.05f}({:,.05f})'.format(np.mean(loss_p_ers) / n, np.std(loss_p_ers) / n)
            else:
                msg += 'loss_c:{:,.05f}'.format(np.mean(loss_c_ers) / n)
                msg += ', loss_p:{:,.05f}'.format(np.mean(loss_p_ers) / n)
            msg += ', erTrain:{:,.5f}, erVal:{:,.5f}'.format(np.mean(train_ers), val_err)
            msg += ', erWeak:{:,.5f}, erAqc:{:,.3f}, erAqa:{:,.3f}'.format(weak_er, er_A_on_qc, er_A_on_qa)
    ...
    for c_size in argv['c_sizes']:
        ...
        for rep_i in range(argv['Bal']['reps']):
            ...
            if os.path.exists(bal_coreset_path):
                ...
                if argv['Bal']['C_init'] == 'subset':
                    valid = False
                    while not valid:  # select from class 0 and class 1 - invalid otherwise
                        C_init = subset_init(c_size, A, trainable=False, to_double=True)
                        U_init = add_cuda(torch.zeros(C_init.shape[0]) + n / C_init.shape[0]).double()  # init U
                        _, C_y = Log.a_to_x_y(C_init)
                        s = torch.sum(C_y).item()
                        valid = -c_size < s < c_size  # can't allow only -ones or only ones
                        if not valid:
                            print('bad selection')
                elif argv['Bal']['C_init'] == 'coreset_init':
                    valid = False
                ...
                U_init.requires_grad = argv['Bal']['U_trainable']
                ...
                if argv['Bal']['loss_stats']:
                    ...
                    print(msg.format(er_A_on_qc, er_A_on_qa, er))
...
def main_balance(argv: dict, A: torch.Tensor, group_names: list):
    print('Balancing:')
    trainQ, valQ, testQ = Log.get_Q_sets(A, argv['Q'])
...
def get_argv(A: torch.Tensor, ds_name: str) -> dict:
    ...
    argv['solver'] = Log.log_solver_sklearn  # solver

    argv['Q'] = {
        'path': '{}/Q.pt'.format(argv['base_folder']),
        ...
        'infoQ_show_er': True,  # prints stats on Q sets (takes time)
        'trainQ_s': 8000,
        'valQ_s': 1600,
        'testQ_s': 800
    }
    ...
    argv['TMF_sp_folder'] = '{}/HTRU_2_tmf.npz'.format(argv['base_folder'])

    # BALANCE:
    argv['Bal'] = {
        'folder': '{}/balance_all'.format(argv['base_folder']),
        # 'folder': '{}/balance_sizes_all'.format(argv['base_folder']),
        ...
        'C_init': 'coreset_init',  # subset , coreset_init
        'U_trainable': False,
        'epochs_info': True,
    ...
    # c_sizes_1 = [100]
    c_sizes_2 = [i for i in range(100, 501, 100)]

    argv['c_sizes'] = c_sizes_2  # TODO c_size
...
def main():
    ...
    main_balance(argv, A, group_names=['SGD', 'TMF', 'UNI'])
    return
